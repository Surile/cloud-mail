<template>
  <div class="settings-container">
    <div
      class="loading"
      :class="firstLoading ? 'loading-show' : 'loading-hide'"
    >
      <loading />
    </div>
    <el-scrollbar class="scroll" v-if="!firstLoading">
        ...
        <div class="card-grid">
          ...
          <div class="settings-card">
            ...
            <div class="card-content">
              ...
              <div class="setting-item">
                ...
                <div class="forward">
                  <span>{{
                    setting.autoCleanDays > 0
                      ? $t("autoCleanRetain", { days: setting.autoCleanDays })
                      : $t("disabled")
                  }}</span>
                  <el-button
                    class="opt-button"
          ...
          <div class="settings-card">
            ...
            <div class="card-content">
              ...
              <div class="setting-item">
                <div>
                  <span>{{ $t("codeRecognitionRules") }}</span>
                </div>
                <div class="forward">
                  <el-button
                    class="opt-button"
            ...
            <div class="card-content">
              ...
              <div class="setting-item">
                <div>
                  ...
                  <el-tooltip
                    effect="dark"
                    :content="$t('applyAutoTrustLevelDesc')"
                  >
                    <Icon
                      class="warning"
                      icon="fe:warning"
                ...
                <el-input-number
                  @change="change"
                  style="width: 110px"
                  v-model="setting.applyAutoTrustLevel"
                  :min="0"
                  :max="9"
                />
              </div>
              ...
              <div class="setting-item">
                ...
                <div>
                  <el-select
                    v-model="setting.zhipuModel"
                    filterable
                    allow-create
                    default-first-option
                    :placeholder="$t('zhipuModelPlaceholder')"
                    :loading="zhipuModelsLoading"
                    style="width: 200px"
                    @focus="loadZhipuModels"
                    @change="saveZhipuModel"
                  >
                    <el-option
                      v-for="m in zhipuModelOptions"
                      :key="m.id"
                      :label="m.id"
                      :value="m.id"
                    />
            ...
            <div class="card-content">
              <div class="concerning-item">
                ...
                <el-badge is-dot :hidden="!hasUpdate">
                  <el-button
                    @click="
                      jump('https://github.com/maillab/cloud-mail/releases')
                    "
                  >
                    {{ currentVersion }}
                    <template #icon>
              ...
              <div class="concerning-item">
                ...
                <div class="community">
                  <el-button
                    @click="jump('https://github.com/maillab/cloud-mail')"
                  >
                    Github
                    <template #icon>
                      <Icon
              ...
              <div class="concerning-item">
                <span>{{ $t("support") }} : </span>
                <el-button
                  @click="jump('https://doc.skymail.ink/support.html')"
                >
                  {{ t("supportDesc") }}
                  <template #icon>
                    <Icon
      ...
      <el-dialog
        v-model="resendTokenFormShow"
        :title="$t('resendToken')"
        width="340"
        @closed="cleanResendTokenForm"
      >
        <form @submit.prevent>
          <el-select
            style="margin-bottom: 15px"
            v-model="resendTokenForm.domain"
            placeholder="Select"
          >
            <el-option
          ...
          <el-input
            ...
            v-model="resendTokenForm.token"
            @keyup.enter="saveResendToken"
          />
          <el-button
            type="primary"
            :loading="settingLoading"
      ...
      >
        <form @submit.prevent>
          <el-input
            ...
            v-model="r2DomainInput"
            @keyup.enter="saveR2domain"
          />
          <el-button
            type="primary"
            :loading="settingLoading"
      ...
      >
        <form @submit.prevent>
          ...
          <el-button
            type="primary"
            :loading="settingLoading"
            @click="saveTurnstileKey"
            >{{ $t("save") }}</el-button
          >
        </form>
      </el-dialog>
      <el-dialog
        v-model="oauthSettingShow"
        :title="$t('oauthSetting') + ' - ' + oauthForm.label"
        width="340"
        @closed="
          oauthForm.clientId = '';
...
Authorization: &lt;secret&gt;</pre
        ...
        <template #footer>
          <div class="dialog-footer">
            ...
            <div class="webhook-footer-right">
              <div
                class="webhook-format-title"
                @click="webhookFormatShow = !webhookFormatShow"
              >
                <span>{{ $t("webhookFormat") }}</span>
                <Icon
                  class="webhook-format-icon"
                  :class="{ open: webhookFormatShow }"
      ...
      <el-dialog
        class="resend-table"
        v-model="showResendList"
        :title="$t('resendTokenList')"
      >
      ...
      <el-dialog v-model="zhipuEditShow" @closed="zhipuKeyInput = ''">
        <template #header>
          <div class="forward-head">
            <span class="forward-set-title">{{ $t("zhipuKeyLabel") }}</span>
          </div>
        </template>
        <el-form>
          :label="t('zhipuKeyInput')" label-position="top"
          <el-input
            type="password"
            show-password
            v-model="zhipuKeyInput"
            :placeholder="$t('zhipuKeyLabel')"
            autocomplete="off"
            @keyup.enter="saveZhipuKey"
          />
        </el-form>
        <el-button
          style="width: 100%"
      ...
      >
        <form @submit.prevent>
          <el-input-number
            type="text"
            v-model="regVerifyCount"
            :min="1"
            @keyup.enter="saveRegVerifyCount"
          >
          </el-input-number>
          <el-button
            type="primary"
            :loading="settingLoading"
            @click="saveRegVerifyCount"
            >{{ $t("save") }}</el-button
          >
        </form>
      </el-dialog>
      ...
      >
        <form @submit.prevent>
          <el-input-number
            type="text"
            v-model="addVerifyCount"
            :min="1"
            @keyup.enter="saveAddVerifyCount"
          />
          <el-button
            type="primary"
            :loading="settingLoading"
            @click="saveAddVerifyCount"
            >{{ $t("save") }}</el-button
          >
        </form>
      </el-dialog>
      ...
      >
        <form @submit.prevent>
          ...
          <div class="s3-button">
            <el-button :loading="clearS3Loading" @click="clearS3">{{
              t("clear")
            }}</el-button>
            <el-button
              type="primary"
              :loading="settingLoading && !clearS3Loading"
              @click="saveS3"
              >{{ t("save") }}</el-button
            >
          </div>
      ...
      >
        <div class="email-prefix">
          <div>{{ t("atLeast") }}</div>
          <el-input-number
            v-model="minEmailPrefix"
            :min="1"
            :max="20"
            style="width: 150px"
            @keyup.enter="saveEmailPrefix"
          >
            <template #suffix>
        ...
        <el-button
          ...
          style="width: 100%"
          :loading="settingLoading"
          @click="saveEmailPrefix"
          >{{ $t("save") }}</el-button
        >
      </el-dialog>
      <el-dialog
        v-model="blackFormShow"
        class="forward-dialog"
        @closed="resetBlackList"
      >
        <template #header>
          <div class="forward-head">
        ...
        <el-form>
          <el-form-item :label="t('blackFromDesc')" label-position="top">
            <el-input-tag
              v-model="blackListForm.blackFrom"
              @add-tag="banEmailAddTag"
            />
          </el-form-item>
      ...
      <el-dialog
        v-model="autoCleanShow"
        :title="t('autoClean')"
        class="forward-dialog"
        @closed="resetAutoClean"
      >
        <el-form>
      ...
      <el-dialog
        v-model="aiCodeFilterShow"
        class="forward-dialog"
        @closed="resetAiCodeFilter"
      >
        <template #header>
          <div class="forward-head">
            <span class="forward-set-title">{{
        ...
        <el-form>
          <el-form-item :label="t('senderRules')" label-position="top">
            <el-input-tag
              v-model="aiCodeFilter"
              @add-tag="aiCodeFilterAddTag"
            />
          </el-form-item>
        </el-form>
        <el-button
...
const tgMsgLabelWidth = computed(() =>
  locale.value === "en" ? "120px" : "100px",
);
...
function getSettings() {
  ...
  settingQuery().then((settingData) => {
    ...
    backgroundUrl.value = setting.value.background?.startsWith("http")
...
const resendList = computed(() => {
  ...
  if (list.length > 0) {
    const key = list.reduce((a, b) =>
      compareByLengthAndUpperCase(a, b, "key"),
    ).key;
    emailColumnWidth.value = getTextWidth(key) + 30;

    const value = list.reduce((a, b) =>
      compareByLengthAndUpperCase(a, b, "value"),
    ).value;
...
function resetBlackList() {
  blackListForm.value.blackFrom = setting.value.blackFrom
    ? setting.value.blackFrom.split(",")
    : [];
  blackListForm.value.blackContent = setting.value.blackContent
    ? setting.value.blackContent.split(",")
    : [];
  blackListForm.value.blackSubject = setting.value.blackSubject
    ? setting.value.blackSubject.split(",")
    : [];
}
...
function resetAiCodeFilter() {
  aiCodeFilter.value = setting.value.aiCodeFilter
    ? setting.value.aiCodeFilter.split(",")
    : [];
}
...
function banEmailAddTag(val) {
  ...
  blackListForm.value.blackFrom.splice(
    blackListForm.value.blackFrom.length - 1,
    1,
  );

  emails.forEach((email) => {
    if (
      (isEmail(email) || isDomain(email)) &&
      !blackListForm.value.blackFrom.includes(email)
    ) {
...
function aiCodeFilterAddTag(val) {
  ...
  emails.forEach((email) => {
    if (
      (isEmail(email) || isDomain(email)) &&
      !aiCodeFilter.value.includes(email)
    ) {
